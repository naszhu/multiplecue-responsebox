...
# Same layout as aug_14_rt_median_by_session_colorCond_sub1-32.R, but color
# encodes participation day (hue) and session order within day (shade).
# Lines still connect the same condition across sessions.
#
# Run from multiplecue-responsebox/ or analysis/:
#   Rscript analysis/aug_14_rt_median_by_session_byParticipationDay_sub1-32.R
...
make_session_condition_plot <- function(page_df, page_tag, subjects_on_page) {
  ...
  legend_info <- build_page_color_map(page_df %>% filter(SubjectNumInt %in% present))
  color_map <- setNames(legend_info$SessionDayColor, legend_info$SessionDayLegend)
  legend_breaks <- legend_info$SessionDayLegend
  ...
  ggplot(page_df, aes(
    x = Session, y = MedianRT,
    color = SessionDayLegendPage, group = ConditionLabel
  )) +
    ...
    labs(
      title = paste0(
        "Median RT by session (by participation day; ", page_tag, ", ", ses_tag, ")"
      ),
      subtitle = paste0(
        "Hue = participation day; lighter-to-darker shade = earlier-to-later session within day ",
        "(lowest session number in a day is lightest). Lines connect conditions. ",
...
for (start_id in page_starts) {
  ...
  p <- make_session_condition_plot(page_df, page_tag, subjects_on_page)
  if (is.null(p)) next
  ...
  out_file <- file.path(
    fig_dir,
    paste0("RTmedian_bySession_byParticipationDay_", page_tag, "_", ses_tag, ".png")
  )
  ggsave(
    filename = out_file,
